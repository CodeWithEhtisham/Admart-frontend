...
> Status: **Frontend wired** — Image Studio calls upload + create job + poll against the backend; see `src/pages/ImageGenPage.jsx` + `src/utils/imageGeneration.js`  
...
> Backend paths are live. FE no longer mocks jobs — it uploads, creates, and polls real `ImageJob`s.
...
| GPT Image 2 | `openai/gpt-image-2` | Best typography | model-specific |

> GPT Image 2 uses the partner namespace `openai/…`, **not** `fal-ai/openai/…`.
...
| GPT Image 2 Edit | `openai/gpt-image-2/edit` | yes |
...
| Label | Model id | Notes |
| ----- | -------- | ----- |
| BiRefNet v2 (default) | `fal-ai/birefnet/v2` | high-res DIS |
| BiRefNet | `fal-ai/birefnet` | light/heavy/portrait |
...
export const TEXT_TO_IMAGE_MODELS = [
  ...
  { id: "openai/gpt-image-2", label: "GPT Image 2", family: "openai" },
] as const;
...
- Credits: show `creditsRemaining` from `/api/auth/me` before generate; optimistic disable when 0.
