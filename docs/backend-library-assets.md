...
- [ ] On image job success: persist durable `sourceUrl` + library row  
- [ ] `GET …/library` with `mediaType` filter + `createdAt DESC`  
- [ ] Do not return temporary fal URLs as the only copy  
- [ ] Scope by `projectId` + owner  
- [ ] Return empty `items: []` (not 404) when the project has no assets  
