...
- **`frontend-design`** (official Anthropic skill) — invoke when building or restyling any UI, to keep visual quality and component consistency. Since the design is largely done, use it to match existing patterns, not reinvent them.
- **`api-design-principles`** — invoke when wiring up the API client/consumer layer, to keep request/response handling and error mapping consistent with the backend contract.
- **`security-auditor`** — **invoke before committing any change that touches auth/token handling, env vars, `dangerouslySetInnerHTML`, or rendering user-generated content.** Treat its findings as blocking.
- **`full-stack-feature`** orchestration — invoke for any feature spanning backend + frontend; use it to consume the backend's new/changed contract and update types → API client → components in one flow.

### Skill guardrails
- Run `security-auditor` before declaring any auth/token/user-content task complete.
- After the backend contract changes, regenerate API types (`npm run gen:api`) before touching the consuming components.
